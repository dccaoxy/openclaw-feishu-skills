...
URL Source: http://lobehub.com/skills/openclaw-skills-feishu-send-file/skill.md

Published Time: Mon, 23 Mar 2026 00:51:31 GMT
...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `openclaw-skills-feishu-send-file`                                |
| Version    | 1.0.2                                     |
| Author     | openclaw                                 |
...
**GitHub:** [openclaw/skills](https://github.com/openclaw/skills) — ⭐ 2596 | Forks: 758

---

## Skill Overview

飞书发送文件技能提供可靠的两步流程：先上传文件获取 file_key，再用 file_key 发送消息。支持推荐的一键脚本或按文档用 curl 手动完成：先用 app_id/app_secret 获取 tenant_access_token，上传文件（file_type=stream）以获取 file_key，然后以 receive_id_type=open_id（个人）或 chat_id（群组）发送 msg_type=file 的消息。适用于 HTML、ZIP、PDF、代码等非图片/视频的普通文件；图片/视频请使用 message tool 的 media 参数。本技能可从 openclaw.json 自动读取凭证，便于在机器人和自动化场景中集成，确保附件正确传递而不是仅显示本地路径。
...
```bash
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-send-file
```
...
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-send-file --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-send-file --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-send-file --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-send-file --version 1.0.2
```
...
```bash
npx -y @lobehub/market-cli skills rate openclaw-skills-feishu-send-file --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment openclaw-skills-feishu-send-file -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment openclaw-skills-feishu-send-file
```
...
```bash
npx -y @lobehub/market-cli skills comments openclaw-skills-feishu-send-file
```
