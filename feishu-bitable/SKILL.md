...
URL Source: http://lobehub.com/skills/openclaw-skills-feishu-bitable-creator/skill.md

Published Time: Mon, 23 Mar 2026 00:27:05 GMT
...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `openclaw-skills-feishu-bitable-creator`                                |
| Version    | 1.0.2                                     |
| Author     | openclaw                                 |
...
Feishu Bitable Creator automates the creation and population of clean, ready-to-use Feishu (Lark) Bitable tables. Use it to create new tables from scratch, convert structured data into table form, batch-create fields and records, and build data tables for research, comparison, or tracking. Key features include automatic cleanup of placeholder rows and default columns, intelligent primary-field naming, and bulk record insertion to speed setup. Authentication and token refresh are handled securely via a pre-configured OpenClaw Feishu channel (no user-supplied API keys), and required app permissions include bitable:app, drive:permission:manage, and drive:drive:read. Security and scope safeguards ensure the skill only manages tables it creates, and adding an admin requires an explicit user_id from conversation context. Ideal for teams needing consistent, fast table provisioning and clean data intake.
...
```bash
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-bitable-creator
```
...
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-bitable-creator --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-bitable-creator --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-bitable-creator --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install openclaw-skills-feishu-bitable-creator --version 1.0.2
```
...
```bash
npx -y @lobehub/market-cli skills rate openclaw-skills-feishu-bitable-creator --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment openclaw-skills-feishu-bitable-creator -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment openclaw-skills-feishu-bitable-creator
```
...
```bash
npx -y @lobehub/market-cli skills comments openclaw-skills-feishu-bitable-creator
```
